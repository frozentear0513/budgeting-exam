...
public class ExamController {
    ...
    @GetMapping("/all")
    @ApiOperation("获取全部试验的列表")
    ResultVO<List<ExamVo>> getExamAll() {
        // 需要拼接前端需要的试验列表对象
        ResultVO<List<ExamVo>> resultVO;
        try {
            List<ExamVo> examVos = examService.getExamAll();
            resultVO = new ResultVO<>(0, "获取全部试验的列表成功", examVos);
        } catch (Exception e) {
            e.printStackTrace();
            resultVO = new ResultVO<>(-1, "获取全部试验的列表失败", null);
        }
        return resultVO;
    }

    @GetMapping("/allUser")
    @ApiOperation("获取用户全部试验的列表")
    ResultVO<List<ExamVo>> getExamAllUser(HttpServletRequest request) {
        // 需要拼接前端需要的试验列表对象
        ResultVO<List<ExamVo>> resultVO;
        String userId = (String) request.getAttribute("user_id");
        try {
            List<ExamVo> examVos = examService.getExamUser(userId);
            resultVO = new ResultVO<>(0, "获取全部试验的列表成功", examVos);
        } catch (Exception e) {
            e.printStackTrace();
            resultVO = new ResultVO<>(-1, "获取全部试验的列表失败", null);
    ...
    ResultVO<Exam> createExam(@RequestBody ExamCreateVo examCreateVo, HttpServletRequest request) {
        ...
        try {
            ...
            resultVO = new ResultVO<>(0, "创建试验成功", exam);
        } catch (Exception e) {
            e.printStackTrace();
            resultVO = new ResultVO<>(-1, "创建试验失败", null);
        }
        return resultVO;
    }
    @PostMapping("/createStudent")
    @ApiOperation("创建试验1")
    ResultVO<ExamStudent> createStudentExam(@RequestBody ExamCreateVo examCreateVo, HttpServletRequest request) {
        // 从前端传参数过来，在这里完成试验的入库
        ResultVO<ExamStudent> resultVO;
        String userId = (String) request.getAttribute("user_id");
        try {
            ExamStudent exam = examService.createStudent(examCreateVo, userId);
            resultVO = new ResultVO<>(0, "创建试验成功", exam);
        } catch (Exception e) {
            e.printStackTrace();
            resultVO = new ResultVO<>(-1, "创建试验失败", null);
    ...
    ResultVO<Exam> updateExam(@RequestBody ExamVo examVo, HttpServletRequest request) {
        ...
        try {
            ...
            resultVO = new ResultVO<>(0, "更新试验成功", exam);
        } catch (Exception e) {
            e.printStackTrace();
            resultVO = new ResultVO<>(-1, "更新试验失败", null);
        }
        return resultVO;
    }

    @GetMapping("/card/list")
    @ApiOperation("获取试验列表，适配前端卡片列表")
    ResultVO<List<ExamCardVo>> getExamCardList() {
        ...
        try {
            ...
            resultVO = new ResultVO<>(0, "获取试验列表卡片成功", examCardVoList);
        } catch (Exception e) {
            e.printStackTrace();
            resultVO = new ResultVO<>(-1, "获取试验列表卡片失败", null);
        }
        return resultVO;
    }

    @GetMapping("/detail/{id}")
    @ApiOperation("根据试验的id，获取试验详情")
    ResultVO<ExamDetailVo> getExamDetail(@PathVariable String id) {
        ...
        try {
            ...
            resultVO = new ResultVO<>(0, "获取试验详情成功", examDetail);
        } catch (Exception e) {
            resultVO = new ResultVO<>(-1, "获取试验详情失败", null);
        }
        return resultVO;
    }

    @PostMapping("/finish/{examId}")
    @ApiOperation("根据用户提交的答案对指定id的试验判分")
    ...
    ResultVO<RecordDetailVo> getExamRecordDetail(@PathVariable String recordId) {
        ...
        try {
            ...
            resultVO = new ResultVO<>(0, "获取试验记录详情成功", recordDetailVo);
        } catch (Exception e) {
            e.printStackTrace();
            resultVO = new ResultVO<>(-1, "获取试验记录详情失败", null);
        }
        return resultVO;
    }

    @GetMapping("/sybgList")
    @ApiOperation("获取全部试验报告列表")
    ResultVO<List<Sybg>> sybgList(HttpServletRequest request) {
//        // 需要拼接前端需要的试验列表对象
//        ResultVO<List<ExamVo>> resultVO;
//        try {
//            List<ExamVo> examVos = examService.getExamAll();
...
//        }
        ...
        String userId = (String) request.getAttribute("user_id");
        // 需要拼接前端需要的试验列表对象
        ResultVO<List<Sybg>> resultVO;
        try {
            List<Sybg> examVos = examService.sybgList(userId);
            resultVO = new ResultVO<>(0, "获取全部试验的列表成功", examVos);
        } catch (Exception e) {
            e.printStackTrace();
            resultVO = new ResultVO<>(-1, "获取全部试验的列表失败", null);
    ...
    @PostMapping("/sybgSave")
    @ApiOperation("创建试验")
    ResultVO sybgSave(@RequestBody Sybg sybg, HttpServletRequest request) {
        // 从前端传参数过来，在这里完成试验的入库
        ResultVO resultVO;
        String userId = (String) request.getAttribute("user_id");
        try {
            sybg.setCreUser(userId);
            resultVO = examService.sybgSave(sybg);
            resultVO = new ResultVO<>(0, "创建试验成功", null);
        } catch (Exception e) {
            e.printStackTrace();
            resultVO = new ResultVO<>(-1, "创建试验失败", null);
        }
