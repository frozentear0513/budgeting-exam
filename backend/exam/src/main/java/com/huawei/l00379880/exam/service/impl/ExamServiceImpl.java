...
public class ExamServiceImpl implements ExamService {
    ...
    private final SybgRepository sybgRepository;

    public ExamServiceImpl(ExamStudentRepository examStudentRepository, QuestionRepository questionRepository, UserRepository userRepository, QuestionLevelRepository questionLevelRepository, QuestionTypeRepository questionTypeRepository, QuestionCategoryRepository questionCategoryRepository, QuestionOptionRepository questionOptionRepository, ExamRepository examRepository, ExamRecordRepository examRecordRepository
    ,SybgRepository sybgRepository) {
        ...
        this.sybgRepository = sybgRepository;
    }
   ...
   @Override
    public List<ExamVo> getExamUser(String userId) {
        List<ExamStudent> examList = examStudentRepository.findUserAll(userId);
        return getExamStudentVos(examList);
    }

    private List<ExamVo> getExamStudentVos(List<ExamStudent> examList) {
        // 需要自定义的exam列表
        List<ExamVo> examVoList = new ArrayList<>();
        // 循环完成每个属性的定制
        for (ExamStudent exam : examList) {
            ...
            for (Question question : radioQuestionList) {
                ...
                radioQuestionVo.setChecked(true); // 试验中的问题肯定被选中的
            ...
            for (Question question : checkQuestionList) {
                ...
                checkQuestionVo.setChecked(true); // 试验中的问题肯定被选中的
            ...
            for (Question question : judgeQuestionList) {
                ...
                judgeQuestionVo.setChecked(true); // 试验中的问题肯定被选中的
    ...
    public Exam create(ExamCreateVo examCreateVo, String userId) {
       ...
       // exam.setExamScore(examScore);
        exam.setExamScore(80);
        examRepository.save(exam);
        return exam;
    }

    @Override
    public ExamStudent createStudent(ExamCreateVo examCreateVo, String userId) {
        // 在线试验系统创建
        ExamStudent examStudent = new ExamStudent();
        BeanUtils.copyProperties(examCreateVo, examStudent);
        examStudent.setExamId(IdUtil.simpleUUID());
        examStudent.setExamCreatorId(userId);
        examStudent.setCreateTime(new Date());
        examStudent.setUpdateTime(new Date());
        // Todo:这两个日志后面是要在前端传入的，这里暂时定为当前日期
        examStudent.setExamStartDate(new Date());
        examStudent.setExamEndDate(new Date());
        String radioIdsStr = "";
        String checkIdsStr = "";
        String judgeIdsStr = "";
        List<ExamQuestionSelectVo> radios = examCreateVo.getRadios();
        ...
        judgeIdsStr = replaceLastSeparator(judgeIdsStr);
        examStudent.setExamQuestionIds(radioIdsStr + "-" + checkIdsStr + "-" + judgeIdsStr);
        // 设置各个题目的id
        examStudent.setExamQuestionIdsRadio(radioIdsStr);
        examStudent.setExamQuestionIdsCheck(checkIdsStr);
        examStudent.setExamQuestionIdsJudge(judgeIdsStr);

        // 计算总分数
        //int examScore = radioCnt * exam.getExamScoreRadio() + checkCnt * exam.getExamScoreCheck() + judgeCnt * exam.getExamScoreJudge();
        // exam.setExamScore(examScore);
        examStudent.setExamScore(80);
    ...
    public Exam update(ExamVo examVo, String userId) {
        ...
        exam.setExamCreatorId(userId); // 试验的更新人为最新的创建人
        exam.setUpdateTime(new Date()); // 试验的更新日期要记录下
    ...
    public ExamRecord judge(String userId, String examId, HashMap<String, List<String>> answersMap) {
        ...
        // 考生作答地每个题目的选项(题目和题目之间用$分隔，题目有多个选项地话用-分隔,题目和选项之间用_分隔),用于查看试验详情
    ...
    @Override
    public List<Sybg> sybgList(String userId) {
        User user = userRepository.findById(userId).orElse(null);
        List<Sybg> examRecordList = new ArrayList<>();
        if(user.getUserRoleId()==3){
            examRecordList = sybgRepository.sybgList(userId);
        }else{
            examRecordList = sybgRepository.sybgAllList();
        }
    ...
    @Override
    public ResultVO sybgSave(Sybg sybg) {
        ResultVO resultVO = new ResultVO();
        try {
            if(StringUtils.isEmpty(sybg.getId())){
                sybg.setId(IdUtil.simpleUUID());
            }
            sybg.setCreTime(new Date());
            sybgRepository.save(sybg);
            resultVO = new ResultVO<>(0, "请求成功", null);
        } catch (Exception e) {
            e.printStackTrace();
            resultVO = new ResultVO<>(-1, "请求失败", null);
        }
        return resultVO;
    }
}
