<template>
  <a-modal title="新建参数" :width="800" :visible="visible" :confirmLoading="confirmLoading" @cancel="handleCancel">
    <a-spin :spinning="confirmLoading">
      ...
      <a-form :form="form">
        ...
        <div v-show="currentStep === 0">
          ...
          <a-form-item label="参数解析" :labelCol="labelCol" :wrapperCol="wrapperCol">
        ...
        <div v-show="currentStep === 1">
          ...
          <a-form-item label="参数归类" :labelCol="labelCol" :wrapperCol="wrapperCol">
        ...
        <div v-show="currentStep === 2">
          ...
          <a-form-item label="说明" :labelCol="labelCol" :wrapperCol="wrapperCol" enterButton="Search">
            <!-- 注意这里要按照单选、多选和判断进行区分 -->
            <!-- 单选 -->
            <a-select
              style="width: 100%"
...
export default {
  ...
  methods: {
    ...
    handleNext (step) {
      ...
      if (currentStep <= 2) {
        ...
        validateFields(stepForms[this.currentStep], (errors, values) => {
          if (!errors) {
            ...
            // 设置参数类型(单选1、多选2或判断3)，用于最后一步问题选项的展示
      ...
      // last step，最后一步，代表完成参数编写，需要点击"完成"创建参数
    ...
    addOption () {
      console.log('添加选项')
      // 这里需要把选项+是否是参数答案作为一个整体添加到options中
      const { form: { validateFields } } = this
      validateFields((errors, values) => { // values就是表单中校验的值，后面提交到后端接口时主要就是用这个values
        console.log('errors:', errors, 'val:', values)
    ...
    handleMultiChange (values) {
      ...
      for (let i = 0; i < this.options.length; i++) { // 遍历所有的参数的选项
        // 取出一个选项的id
