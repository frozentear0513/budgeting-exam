    ...
    <a-layout-header class="header" style="color: #fff">
      ...
      <span style="float: right;">
        <span style="margin-right: 40px; font-size: 20px" v-if="recordDetail.examRecord">
          实验得分：<span style="color: red">{{ recordDetail.examRecord.examJoinScore }}</span>&nbsp;分&nbsp;
          <span style="font-size:15px;">参加实验时间：{{ recordDetail.examRecord.examJoinDate }}</span>
        </span>
      ...
      <a-layout :style="{ marginLeft: '200px' }">
        <a-layout-content :style="{ margin: '24px 16px 0',height: '84vh', overflow: 'initial' }">
          <div :style="{ padding: '24px', background: '#fff',height: '84vh'}">
            <span v-if="currentQuestion === ''" style="font-size: 30px;font-family: Consolas">欢迎查看本次实验情况，点击左侧参数编号可以查看答题详情</span>
        ...
        <a-layout-footer :style="{ textAlign: 'center' }">
          Spting Boot Virtual test platform ©2020 Created by Liang Shan Guang
        </a-layout-footer>
      </a-layout>
    </a-layout>
  </a-layout>
...
export default {
  ...
  mounted () {
    ...
    const that = this
    // 从后端获取实验的详细信息，渲染到实验详情里,需要加个延时，要不拿不到参数
      ...
      getQuestionDetail(questionId)
        .then(res => {
          if (res.code === 0) {
            ...
            // 查看用户是不是已经做过这道题又切换回来的，answersMap中查找，能找到这个参数id对应的值数组不为空说明用户做过这道题
